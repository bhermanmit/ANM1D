...
class StatePoint(object):
    ...
    def extract_results(self, tally_id, score_str, nuclide_id = -1):
        """Returns a tally results dictionary given a tally_id and score string.
           ...
           nuclide_id: int
               -1: For total material (Default value)
                ZAID for actual isotope
        """
        ...
        # account for nuclide filters
        nuclide_list = list(tally.nuclides)
        try:
            nuclide_idx = nuclide_list.index(nuclide_id)
        except ValueError:
            print 'Nuclide does not exist'
            print tally.nuclides
            return
        idx = idx + nuclide_idx*n_scores 
